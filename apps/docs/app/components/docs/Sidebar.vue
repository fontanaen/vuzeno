...
function scrollActiveItemIntoView() {
  const content = sidebarRef.value?.querySelector<HTMLElement>('[data-sidebar="content"]');
  ...
  const itemOffset = itemRect.top - contentRect.top + content.scrollTop;
  const targetScroll = itemOffset - content.clientHeight / 2 + itemRect.height / 2;
...
<template>
  <div ref="sidebarRef" class="contents">
    <Sidebar.Root
