...
export function usePrevNext() {
  ...
  return computed(() => {
    ...
    const links = getFlatSideBarLinks(sidebar).filter((link) => !link.link.includes("llms.txt"));
    ...
    return {
      prev: hidePrev
        ...
        : {
            text:
              (typeof frontmatter.value.prev === "string" ? frontmatter.value.prev : typeof frontmatter.value.prev === "object" ? frontmatter.value.prev.text : undefined) ??
      ...
      next: hideNext
        ...
        : {
            text:
              (typeof frontmatter.value.next === "string" ? frontmatter.value.next : typeof frontmatter.value.next === "object" ? frontmatter.value.next.text : undefined) ??
              candidates[index + 1]?.docFooterText ??
              candidates[index + 1]?.text,
