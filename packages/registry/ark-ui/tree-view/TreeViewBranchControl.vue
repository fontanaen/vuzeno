...
import { TreeView, type TreeViewBranchControlProps } from "@ark-ui/vue/tree-view";
import { reactiveOmit } from "@vueuse/core";
import { cn } from "cnfast";
import type { HTMLAttributes } from "vue";

const props = defineProps<TreeViewBranchControlProps & { class?: HTMLAttributes["class"] }>();

const branchControlProps = reactiveOmit(props, "class");
</script>

<template>
  <TreeView.BranchControl
    v-bind="branchControlProps"
    :class="cn(
      'group/row relative flex w-full cursor-pointer items-center gap-2 rounded-md border-none bg-transparent font-[inherit] text-foreground text-start select-none',
